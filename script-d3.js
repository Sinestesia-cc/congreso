...
function init() {
  ...
  // UI oculta por defecto para Touch Designer
  console.log("🔇 UI oculta por defecto (Touch Designer ready)");
  window.touchDesignerAPI.toggleUI(true);

  // Asegurar que el botón STOP esté oculto al inicio
  d3.select("#stop-button").style("display", "none");
}
    ...
    importButton.addEventListener("click", () => {
      if (importInput.style.display === "none" || importInput.style.display === "") {
        ...
        try {
          ...
          updateColumnPositions();
          if (selectedYears.length > 0 || sankeyDrawn) {
            console.log(" Redibujando diagrama con nueva configuración importada");
            drawSankey(selectedYears.length > 0 ? selectedYears : [selectedYears[0] || getUniqueYears(allData)[0]]);
          }
...
function createYearButtons(years) {
  ...
  years.forEach(year => {
    const button = container.append("button")
      ...
      .text(year)
      .classed("active", selectedYears.includes(year))
      .on("click", function() {
...
function prepareSankeyData(years) {
  // Soporte para años únicos o múltiples
  if (!Array.isArray(years)) {
    years = [years];
  }

  const yearData = allData.filter(d => years.includes(d.year));
...
function drawSankey(years) {
  // Soporte para años únicos (compatibilidad hacia atrás) o múltiples
  if (!Array.isArray(years)) {
    years = [years];
  }

  selectedYears = years; // Mantener sincronizado
  console.log("Dibujando Sankey para los años:", years);
  ...
  d3.select(".title")
    .text(years.length === 1 ? `Congreso ${years[0]}` : `Comparación: ${years.join(" vs ")}`)
    .style("font-family", CONFIG.fontFamily);
  
  d3.select("#loading").style("display", "block");
  
  const sankeyData = prepareSankeyData(years);
  ...
  [0, 1, 2, 3].forEach(column => {
    if (linksByColumn[column]) {
      linksByColumn[column].forEach(({ element, link }, localIndex) => {
        ...
        // Si hay dos años seleccionados, agregar delay adicional para el segundo año
        if (selectedYears.length === 2) {
...
window.clearSelectedYears = function() {
  ...
  console.log("🧹 Años limpiados - interfaz lista para aplicación padre");
};
...
window.touchDesignerAPI = {
  ...
  toggleUI: function(hide) {
    ...
    elementsToToggle.forEach(selector => {
      ...
      if (!element.empty()) {
        // El botón STOP solo se muestra si hay diagrama dibujado
        if (selector === "#stop-button") {
          element.style("display", shouldHide ? "none" : (sankeyDrawn ? "block" : "none"));
        } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (playButton) {
    playButton.addEventListener("click", () => {
      if (selectedYears.length > 0) {
        ...
        alert("Selecciona al menos un año antes de presionar Play");
      }
    });
  }
  ...
  // Botón Toggle UI - Oculta/muestra toda la interfaz
  const toggleUIButton = document.getElementById("toggle-ui-button");
  if (toggleUIButton) {
    let uiHidden = true; // Inicia como true porque la UI está oculta por defecto
    toggleUIButton.addEventListener("click", () => {
      ...
      const elementsToToggle = [
        ...
        "#clear-config",
        "#test-save",
        ".title" // Ocultar el título del diagrama
      ];
      ...
      elementsToToggle.forEach(selector => {
        ...
        if (!element.empty()) {
          // El botón STOP solo se muestra si hay diagrama dibujado
          if (selector === "#stop-button") {
            element.style("display", uiHidden ? "none" : (sankeyDrawn ? "block" : "none"));
          } else {
            element.style("display", uiHidden ? "none" : null);
          }
        }
      });

      // Cambiar texto del botón
      toggleUIButton.textContent = uiHidden ? "Mostrar UI" : "UI";
      toggleUIButton.style.background = uiHidden ? "rgba(0,0,0,0.7)" : "rgba(0,0,0,0.9)";

      console.log(uiHidden ? "UI oculta - Modo proyección" : "UI visible - Modo edición");
    });
  }
});